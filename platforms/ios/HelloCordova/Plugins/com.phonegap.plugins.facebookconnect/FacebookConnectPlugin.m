...
- (void)login:(CDVInvokedUrlCommand *)command {
    ...
    if (FBSession.activeSession.isOpen) {
        ...
        if ([self areAllPermissionsReadPermissions:permissions]) {
            ...
            NSUInteger behaviourType = FBSessionLoginBehaviorWithNoFallbackToWebView;

            if (![UIApplication.sharedApplication canOpenURL:[NSURL URLWithString:@"fb://"]])
            {
                behaviourType = FBSessionLoginBehaviorForcingWebView;
            }

            [FBSession setActiveSession: [[FBSession alloc] initWithPermissions:permissions] ];
            [[FBSession activeSession] openWithBehavior:behaviourType
                                      completionHandler:^(FBSession *session,
                                                        FBSessionState state,
                                                        NSError *error) {
                                [self sessionStateChanged:session
